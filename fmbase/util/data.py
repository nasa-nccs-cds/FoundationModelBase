...
def extract_input_target_times(
  ...
  print(f"  -->  target_lead_times = {target_lead_times}")
  ...
  shifted_time = time + target_duration - time[-1]
  dataset = dataset.assign_coords( time=shifted_time )
  print(f" ***  time: shape={time.shape}, sample={time[:10]}")
  print(f" *** stime: shape={shifted_time.shape}, sample={shifted_time[:10]}")
